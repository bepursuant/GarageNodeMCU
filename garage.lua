...
conf.open = 6
conf.closed = 7
conf.delay = 200000 --how long to hold relay open to trigger door opener
...
module.start = function()
  ...
  if conf.relay and conf.open and conf.closed and conf.delay then
    ...
    gpio.mode(conf.relay, gpio.OUTPUT)
    ...
    gpio.mode(conf.open, gpio.INPUT, gpio.PULLUP)
    gpio.trig(conf.open, "both", module.onOpenChange)
    ...
    gpio.mode(conf.closed, gpio.INPUT,gpio.PULLUP)
    gpio.trig(conf.closed, "both", module.onCloseChange)
   
  else
    print("GarageNodeMCU Configuration is missing or invalid, check the header of garage.lua")
  end
end
...
module._toggle = function()
  gpio.write(conf.relay, gpio.HIGH)
  tmr.delay(conf.delay)
  gpio.write(conf.relay, gpio.LOW)
  return true
end

-- check the current status of the sensors
module.status = function()
  local open = gpio.read(conf.open)
  local closed = gpio.read(conf.closed)
